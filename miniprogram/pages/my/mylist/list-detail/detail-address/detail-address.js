...
Page({
  ...
  onLoad(options) {
    var that = this;
    var latitude = options.current_latitude;
    var longitude = options.current_longitude;
    console.log(options);
    this.setData({
      markers: [{ id: 1, iconPath: '../../../../../images/marker2.png', longitude:longitude, latitude:latitude}]
    })
  ...
  back(e) {
    ...
    console.log(searchList[addressid])
    var pages = getCurrentPages();//当前页面栈
    if (pages.length > 1) {
      var beforePage = pages[pages.length - 2];//获取上一个页面实例对象
      beforePage.setData({
        ['post.area']: searchList[addressid].name + searchList[addressid].address,
        location: searchList[addressid].location
      });//触发父页面中的方法
    };
    var history = wx.getStorageSync("historyList") || [];
    history.push(searchList[addressid])
    wx.setStorageSync("historyList", history);
    wx.navigateBack({
      delta: 1
    })
  ...
  back2(e) {
    ...
    if (pages.length > 1) {
      var beforePage = pages[pages.length - 2];//获取上一个页面实例对象
      beforePage.setData({
        ['post.area']: historyList[historyid].name + historyList[historyid].address,
        location: historyList[historyid].location
      });//触发父页面中的方法
    };
