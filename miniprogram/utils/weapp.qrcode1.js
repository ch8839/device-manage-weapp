  ...
  function createCanvas() {
    ...
    for (var row = 0; row < qrcode.getModuleCount(); row++) {
      for (var col = 0; col < qrcode.getModuleCount(); col++) {
        ...
        ctx.fillRect(Math.round(col * tileW)+5, Math.round(row * tileH)+10, w, h);

        ctx.save();
        ctx.translate(180, 60)
        ctx.setFillStyle('#383c42')  // 文字颜色：黑色
        ctx.setFontSize(16) 
        ctx.rotate(90 * Math.PI / 180) 
        ctx.fillText(_id, 0, 0)
        ctx.restore();

        ctx.setFillStyle('#383c42')  // 文字颜色：黑色
        ctx.setFontSize(16)         // 文字字号：22px
        
        ctx.fillText(content, 52, 195)
      }
    }
